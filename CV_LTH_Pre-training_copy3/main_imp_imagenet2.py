...
def main():
    ...
    # Data loading code
    traindir = os.path.join(args.data, 'imagenet-c','gaussian_noise','2')
    ...
    for state in range(start_state, args.pruning_times):
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            test_tacc = test(val_loader, model, criterion, args)
        ...
        if args.random_prune:
            pruning_model_random(model.module, args.rate, conv1=args.conv1)
        else:
            pruning_model(model.module, args.rate, conv1=args.conv1)

        current_mask = extract_mask(model.state_dict())

        remove_prune(model.module, conv1=args.conv1)
        #rewind weight to init
        check_sparsity(model.module, conv1=args.conv1)
        model.load_state_dict(initalization)
        prune_model_custom(model.module, current_mask, conv1=args.conv1)
